// Admin Edit Account Page JavaScript
...
function saveUserData(updatedUser) {
    try {
        ...
        // Update admin officers array
        const adminOfficers = JSON.parse(localStorage.getItem('adminOfficers') || '[]');
        const index = adminOfficers.findIndex(admin => admin.id === updatedUser.id);
        if (index !== -1) {
            adminOfficers[index] = {
                ...adminOfficers[index],
            ...
            localStorage.setItem('adminOfficers', JSON.stringify(adminOfficers));
        }
